...
visibility, the `using`-in-body rejection, and Ractor concerns.  The
model captures what a refined proc *means*, not how the implementation
caches it.
...
| --- | --- | --- |
| `staged_eq_oneshot` | Activating `a ++ b` equals activating `a`, then `b`, from the resulting table | yes (the staged side is chaining) |
...
| Model | CRuby |
| --- | --- |
| `apply_seq` | `rb_using_module_recursive` accumulating refinements in a cref |
...
The implementation memoizes one iseq copy per (state, modules) key, so
`p.refined(a).refined(b)` and `p.refined(a, b)` produce *distinct*
copies with equal behavior: the model's equalities correspond to
observational equivalence of procs, not object identity.
